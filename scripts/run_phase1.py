...
def main():
    ...
    print("=" * 50)
    print(f"ML: {[round(x, 4) for x in ml_test_scores]} | Mean: {np.mean(ml_test_scores):.4f} ± {np.std(ml_test_scores):.4f}")
    print(f"DL: {[round(x, 4) for x in dl_test_scores]} | Mean: {np.mean(dl_test_scores):.4f} ± {np.std(dl_test_scores):.4f}")
